...
#
#さい銭箱入金処理 koko2005/09/01

# リンクのボタンを付ける('yes','no')
$linkbotan = 'no';

# 家を売却の時運営会社を消してしまう。 ('yes','no') koko2007/06/17
$baikyku_syoumetu = 'no';

# 特殊の家使いナンバー(0から始まる) %ie_hash2
$tokusyu_ie_no = '';
# eval{ flock (IN, 2); }; ロック強化　2007/06/17
...
#require './unei.pl'; #koko2007/04/20　使用していない。
require './unei_2.pl'; #koko2007/04/30　場所移動
require './kaishiya.pl'; #koko2007/05/05 場所移動
...
#店を持っていない時　#koko2007/03/22
	if(!($my_con1 eq "1" || $my_con2 eq "1" || $my_con3 eq "1" || $my_con4 eq "1")){ #koko2007/03/23修正
...
#koko2007/08/01
	...
	●街で家にマウスがのった時に表\示されるコメント（40字以内）<br>
EOM
...
EOM
		...
		if ($ori_ie_rank2 != 3) {print "タイトル <input type=text name=\"タイトル$_\" value=\"$oriie_settei_sprit[$titlecount]\">";}
			$selectcount ++;
			$titlecount ++;
		}
#koko2007/08/15
...
<input type=hidden name=k_id value="$in{'k_id'}">
	<input type=hidden name=town_no value="$in{'town_no'}">
	●家の外観、内装（コンテンツ内容）の変更<br>
	場所はそのままに、家の外観を変更したり、内装をアップグレードすることができます。<br>
	<input type=submit value=" 選択画面へ "></form>
EOM
		$ieari_f = '';
...
<input type=hidden name=iesuu value="0">
	...
	場所はそのまま、家1の外観を変更
	<input type=submit value=" 選択画面へ1"></form>
EOM
		}
		$ieari_f = '';
...
<input type=hidden name=iesuu value="1">
	...
	場所はそのまま、家2の外観を変更
	<input type=submit value=" 選択画面へ2"></form>
EOM
		}
		$ieari_f = '';
...
<input type=hidden name=iesuu value="2">
	...
	場所はそのまま、家3の外観を変更
	<input type=submit value=" 選択画面へ3"></form>
EOM
		}
		$ieari_f = '';
...
<input type=hidden name=iesuu value="3">
	...
	場所はそのまま、家の外観を変更
	<input type=submit value=" 選択画面へ4"></form>
EOM
		}
		print "<hr size=1>";
#kokoend
...
<input type=hidden name=k_id value="$in{'k_id'}">
	<input type=hidden name=town_no value="$in{'town_no'}">
	●家の売却<br>
	家の場所を変更したい場合は、一度家を売却してから再度購入してくだい。売却で得られるのは土地の価格だけです。現在のコンテンツ内容は新たに家を購入した場合も保持されています。<br>
	<input type=submit value=" 家の売却 "></form>
...
###コメント変更
sub comment_change {
	if (length($in{'ori_ie_setumei'}) > 100) {&error("コメントは50字以内です");}
...
sub my_house_settei_do {
	...
	$oriie_settei_file="$my_directry/oriie_settei.cgi";
		...
		$my_con3 = $in{'コンテンツ3'};
		$my_con4 = $in{'コンテンツ4'};
		$my_con1_title = $in{'タイトル1'};
		$my_con2_title = $in{'タイトル2'};
		$my_con3_title = $in{'タイトル3'};
		$my_con4_title = $in{'タイトル4'};
...
<input type=hidden name=k_id value="$in{'k_id'}">
	...
	●掲示板のタイトル（タグ指定可。絶対URLならばイメージ画像の指定も可）<br>
	<textarea  cols=80 rows=4 name="タイトル" wrap="soft">$gentei_title</textarea><br>
	●タイトル下のコメント<br>
	<input type=text name="コメント" size=120  value=$gentei_come><br>
	●背景のスタイル設定<br>
...
<input type=hidden name=k_id value="$in{'k_id'}">
	...
	<textarea  cols=80 rows=4 name="タイトル" wrap="soft">$omise_title</textarea><br>
...
EOM
	...
	<br>
	●お店のコメント<br>
	<textarea  cols=80 rows=3 name="コメント" wrap="soft">$omise_come</textarea><br>
...
<input type=hidden name=k_id value="$in{'k_id'}">
	...
	<textarea  cols=80 rows=4 name="タイトル" wrap="soft">$dokuzi_title</textarea><br>
	●URL指定<br>
	<input type=text name="dokuzi_url" size=120 value="$dokuzi_url"><br>
	●タイトル下のコメント<br>
	<input type=text name="コメント" size=120  value=$dokuzi_come><br>
	●背景のスタイル設定<br>
...
#koko2007/05/02-04
			...
			#	($ori_ie_para_moto,$akichi) = split(/=/,$town_sprit_matrix[$my_point_is]); #koko2007/05/04
				if (!$akichi){$akichi = "空地";}
	#			if ($ori_ie_town eq "3"){ #koko2007/03/29 街の番号
	#				if (!$akichi){$akichi = "空地2";}
	#			}else{
	#				if (!$akichi){$akichi = "空地";}
	#			}
...
<input type=hidden name=town_no value="$in{'town_no'}">
<input type=image src="./img/go.gif" alt="掲示板に飛びます。">
$b_name</form>
EOM
			}else{
				print "$b_name";
				...
				print <<"EOM";
</span>：$b_com</xmp>（$b_date）<span style=\"font-size:9px\">記事no.$b_mail</span></div>
EOM
#kokoend
		}else{
...
EOM
			...
			print <<"EOM";
</span>：$b_com</span></xmp>（$b_date）<span style=\"font-size:9px\">記事no.$b_mail</span>
	<form method="POST" action="$this_script">
	<input type=hidden name=mode value="bbs_regist">
	<input type=hidden name=name value="$in{'name'}">
	<input type=hidden name=job value="$job">
...
EOM
#koko2006/12/12 上　text <textarea に変更
		}
	}		#foreach閉じ
...
<input type=hidden name=k_id value="$in{'k_id'}">
	...
	<input type=hidden name=bbs_num value="$in{'bbs_num'}">
	<input type=hidden name=town_no value="$in{'town_no'}">
	</xmp>記事no. <input type=text name=b_count size=8>
...
#ログファイル更新
	...
	if (length($in{'b_com'}) > 2000) {&error("挨拶は全角1000字半角2000字以内です");} #koko2006/12/12
...
# コメントの改行処理
	...
	$in{'b_com'} =~ s/\r/<br>/g;
	$in{'b_com'} =~ s/\n/<br>/g;
	$in{'b_com'} =~ s/<plaintext/System：plaintextタグの使用はやめましょう。/gi;
	$in{'b_com'} =~ s/<script/System：JavaScriptは危ないから禁止。ごめんね。/gi;
...
#新規投稿なら新記事Noを取得
	if ($in{'b_res'} eq ""){
		$total_counter++;
#更新配列を定義
		$new_toukou = "$total_counter<>$in{'name'}<span style=\"font-size:9px\">（$in{'job'}）</span><>$date2<>$in{'b_res'}<>$all_total_counter<>$in{'b_com'}<>$in{'k_id'}<>\n"; #koko2006/12/11		#ver.1.40
...
#更新配列を定義
			$new_toukou = "<>$in{'name'}<span style=\"font-size:9px\">（$in{'job'}）</span><>$date2<>$in{'b_res'}<>$all_total_counter<>$in{'b_com'}<>$in{'k_id'}<>\n";		#ver.1.40 #koko2006/12/11
		foreach (@all_data){
...
#お金をゲット
	...
	if ($randed == 7){
		...
		$money += $randed;
		$message_in = "●ボーナスです！$randed円のお金をゲットしました。";
	}else{
